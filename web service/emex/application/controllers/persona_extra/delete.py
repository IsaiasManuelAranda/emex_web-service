...
class Delete:
    ...
    def GET(self, id_persona, **k):
        ...
        result = config.model.get_persona_extra(int(id_persona)) # search  id_persona
    ...
    def POST(self, id_persona, **k):
        ...
        result = config.model.delete_persona_extra(form['id_persona']) # get persona_extra data
        if result is None: # delete error
            message = "El registro no se puede borrar" # Error messate
            id_persona = config.check_secure_val(str(id_persona))  # HMAC user validate
            id_persona = config.check_secure_val(str(id_persona))  # HMAC user validate
            result = config.model.get_persona_extra(int(id_persona)) # get id_persona data
            result.id_persona = config.make_secure_val(str(result.id_persona)) # apply HMAC to id_persona
            return config.render.delete(result, message) # render delete.html again
        else:
            raise config.web.seeother('/persona_extra') # render persona_extra delete.html 
